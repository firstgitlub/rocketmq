...
public class BrokerController {
    ...
    public BrokerController(
        ...
        //consumer 消费offset 处理类
        this.consumerOffsetManager = new ConsumerOffsetManager(this);
        //管理topic配置
        this.topicConfigManager = new TopicConfigManager(this);

        //需要关注这个两个类是干什么的
        //这个是进行真正的对于pullRequest请求 当有数据到达时 做的真正获取数据并下发到客户端consumer
        this.pullMessageProcessor = new PullMessageProcessor(this);
        //这个类就是长轮询的实现类
        this.pullRequestHoldService = new PullRequestHoldService(this);

        //这类类也是很关键的
        //消息达到监听器 会调用pullRequestHoldService 的notifyMessageArriving 方法进行消息下发
    ...
    public boolean initialize() throws CloneNotSupportedException {
        ...
        if (result) {
            try {
                //DefaultMessageStore broker 存/取 消息使用
                //消息存储管理组件 管理磁盘上的消息
                this.messageStore =
                    new DefaultMessageStore(this.messageStoreConfig, this.brokerStatsManager, this.messageArrivingListener,
                        this.brokerConfig);

                //如果启动了Dledger,就初始化相关组件
        ...
        if (result) {
            //broker 作为服务端：接受produce consumer发送来的请求
            //broker 也作为客户端  想nameServer发送心跳通知
            //这里面开始初始化 Netty 服务端
            ...
            /**
             * broker启动时候 会注册很多processor
             * 用于处理 netty客户端发送的网络事件
             */
            ...
            //filter执行
            //consumer端把消息的filter处理程序上传到了broker端，这样可以减少一些网络请求
            this.scheduledExecutorService.scheduleAtFixedRate(new Runnable() {
                @Override
                public void run() {
                    try {
    ...
    public void registerProcessor() {
         ...
         *
         * //asyncProcessRequest 的 SendMessageProcessor
         */
    ...
    /**
     * 这里面这么多start 需要做仔细分析的
     */
    public void start() throws Exception {
        if (this.messageStore != null) {
            /**
             * 消息存储服务 {@link DefaultMessageStore}
             */

            // AbstractPluginMessageStore 这个类对 DefaultMessageStore 进行了包装，没看出啥目的
        ...
        //broker 服务端在消息到达时  通知pullMessage
        /**
         * 猜测应该是push 模式，在轮训消息达到后
         * 从commitLog中获取消息 然后通过netty发送到客户端
         */
        if (this.pullRequestHoldService != null) {
        ...
        if (this.filterServerManager != null) {
            /**
             * { @link https://blog.csdn.net/sinat_28771747/article/details/85198927}
             * consumer 与 broker 交互,通过consumer上传的 filter，过滤一些broker发送到consumer 的一些数据
             */
