...
public class PullMessageProcessor extends AsyncNettyRequestProcessor implements NettyRequestProcessor {
    ...
    //这里处理的就是 长轮询 pullRequestHoldService 类做的事情吧，把consumer端的pullRequest 数据下发到对应的channel上
    private RemotingCommand processRequest(final Channel channel, RemotingCommand request, boolean brokerAllowSuspend)
        ...
        //这里是真正的从MessageStore中获取消息
        /**
         * 1:CommitLog 消息的物理存储位置
         * 2:ConsumeQueue 消息的逻辑队列
         */
        ...
        if (getMessageResult != null) {
            ...
            //这里为什么要设置从 从服务器拉数据？  是为了上面的拉取结果失败做重试用的吗？
            if (getMessageResult.isSuggestPullingFromSlave()) {
                //当消费缓慢的时候建议从 slave pull message
            ...
            switch (response.getCode()) {
                case ResponseCode.SUCCESS:
                    ...
                    /**
                     * transferMsgByHeap 要理解该字段是做什么的：
                     *
                     * 是否通过堆内存传输数据,正常情况下是需要开启的
                     *
                     *  <li>
                     *      https://blog.csdn.net/gaoliang1719/article/details/106991065
                     *  </li>
                     *
                     */
                    if (this.brokerController.getBrokerConfig().isTransferMsgByHeap()) {
                        ...
                        try {
                            /**
                             * ManyMessageTransfer 类的存在，
                             * 这个在上篇分享的时候提到过叫页缓存与内存映射,当然提到页缓存技术不得不提的就是零拷贝
                             *
                             * <li>
                             *     https://blog.csdn.net/nihui123/article/details/105409661
                             * </li>
                             */
                            FileRegion fileRegion =
                                new ManyMessageTransfer(response.encodeHeader(getMessageResult.getBufferTotalSize()), getMessageResult);

                            //这里实现真正把数据通过channel 发送给消费端
                ...
                case ResponseCode.PULL_NOT_FOUND:
                    ...
                    if (brokerAllowSuspend && hasSuspendFlag) {
                        ...
                        /**
                         * 也就是在第二次触发事件的时候，调用了PullRequestHoldService服务，
                         * 并调用了其中的suspendPullRequest()方法。这个方法其实就是将ManyPullRequest进行了封装。
                         */
                ...
                try {
                    ...
                    if (response != null) {
                        ...
                        try {
                            //这里怎么写两次呢？？？  这个是给谁写的？
                            /**
                             * 解了：因为PullMessageProcessor.this.processRequest 里面当通过channel执行了通信的话
                             * 返回的response 是null ,就不会再走这个逻辑了，
                             * over
                             */
