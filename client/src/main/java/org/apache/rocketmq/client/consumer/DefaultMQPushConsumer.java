...
public class DefaultMQPushConsumer extends ClientConfig implements MQPushConsumer {
     ...
     * 相同角色的消费者需要拥有完全相同的订阅和消费组，
     * 才能正确地实现负载平衡。这是必需的，并且需要是全局唯一的。请参阅此处进行进一步讨论。
     */
     ...
     * </ul>
     * 消费者引导上的消费点。有三个消费点:
     * CONSUME_FROM_LAST_OFFSET:消费者客户端从之前停止的地方开始。如果它是一个新启动的消费者客户端，根据消费者群体的年龄，有两种情况:
     * 如果消费者组是最近创建的，最早订阅的消息还没有过期，这意味着该消费者组代表最近启动的业务，那么消费将从头开始;
     * 如果订阅的最早的消息已经过期，那么消费将从最新的消息开始，这意味着在引导时间戳之前生成的消息将被忽略。
     * CONSUME_FROM_FIRST_OFFSET:客户端将从可用的最早消息开始。
     * CONSUME_FROM_TIMESTAMP:客户端将从指定的时间戳开始，这意味着在consumeTimestamp之前产生的消息将被忽略
     */
     ...
     *
     * 回溯消耗时间与第二精度。时间格式为20131223171201，暗示2013年12月23日1712秒01。
     * 默认回溯消耗时间半小时前。
     */
    ...
    /**
     *
     * 将消息发送回broker，该broker将在将来被重新发送。
     * 2020年4月5日以后，该方法会被删除，或者会在某一版本中改变其可见性，所以请不要使用该方法。
     */
    @Deprecated
    @Override
