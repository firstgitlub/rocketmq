...
public class MQClientAPIImpl {
    ...
    public MQClientAPIImpl(final NettyClientConfig nettyClientConfig,
        final ClientRemotingProcessor clientRemotingProcessor,//这里猜测是NettyRemotingClient 调用后对响应做异步处理用的
        RPCHook rpcHook,
        final ClientConfig clientConfig) {

        //DefaultMQProducer 继承了 ClientConfig clientConfig = DefaultMQProduce
        this.clientConfig = clientConfig;
        //topAddressing  这个类什么用处？
        // namesrv地址信息获取地址
        topAddressing = new TopAddressing(MixAll.getWSAddr(), clientConfig.getUnitName());
        //用于通信使用的 RemotingClient
        //有NettyRemotingClient   和 NettyRemotingServer
        //远程通信客户端
        this.remotingClient = new NettyRemotingClient(nettyClientConfig, null);
        //clientRemotingProcessor 最终还是注册到了NettyRemotingClient 上面
        /**
         * 当NettyRemotingClient 接收到来自于服务器端发送来的数据，处理使用的是 clientRemotingProcessor 对应的业务逻辑处理
         * 依据是：不同的 RequestCode 有对应的处理方式
         */
        this.clientRemotingProcessor = clientRemotingProcessor;

        //rpcHook 是在执行网络通信之前和 之后进行的数据包装 拆装
        //目前Hook是null
        this.remotingClient.registerRPCHook(rpcHook);
        //这里面注册的各种处理器，应该是怎么不通的场景功能 所做的一些处理。猜测
        /**
         * 服务端BrokerController 启动的时候会注册很多Processor 是要处理server接受客户端不同requestCode 对应的processor
         * 同样，客户端在启动的时候也会注册很多 processor 处理服务端发到客户端的不通requestCode 对应的processor
         */
        /**
         * executor 线程池 去执行 clientRemotingProcessor里对应的方法
         */
        //远程通信客户端注册处理器
        ...
        try {
            ...
            if (addrs != null) {
                if (!addrs.equals(this.nameSrvAddr)) {
                    ...
                    //这个更新可以理解为 更新本地 缓存中的nameServerList
    ...
    //produce 在发送消息的时候 正常是同步发送 因为需要根据响应结果 判断是否需要重试
    //因为只有是同步发送 失败才有重试机制 默认是三次
    ...
    public void updateConsumerOffset(
        ...
        //更新客户端offset到远端broker  这种是在消息模式为 集群消息的情况下才会向远端去同步数据
        //如果是广播消息的话 只是在本地同步offset
    ...
    public TopicRouteData getTopicRouteInfoFromNameServer(final String topic, final long timeoutMillis,
        ...
        switch (response.getCode()) {
            ...
            case ResponseCode.SUCCESS: {
                //网络间通信都是二进制字节码数据 01010101
                byte[] body = response.getBody();
                if (body != null) {//需要解码对应的实体对象  底层对象字节码转换使用的是 FastJson
