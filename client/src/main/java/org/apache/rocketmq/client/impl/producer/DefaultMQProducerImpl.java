...
public class DefaultMQProducerImpl implements MQProducerInner {
     ...
     *
     * 它将在4.4.0时被删除，原因是异常处理和错误的超时语义。下一个版本将提供一个新的
     * 参数:
     * 味精,
     * sendCallback -
     * timeout - sendCallback将在大多数时间被调用
     * 抛出:
    ...
    private SendResult sendDefaultImpl(
        ...
        if (topicPublishInfo != null && topicPublishInfo.ok()) {
            ...
            for (; times < timesTotal; times++) {
                ...
                //选择队列
                /**
                 * topic 创建的时候 是可以选择对应的broker broker中包含对应的queue 正常是8个，4个读4个写
                 * 创建的topic 可以执行broker 以及队列的数量 ，消息存储的时候是存在 对应的 broker 的 queue中
                 */
    ...
    private TopicPublishInfo tryToFindTopicPublishInfo(final String topic) {
        ...
        if (null == topicPublishInfo || !topicPublishInfo.ok()) {
            ...
            //这里和nameServer 对应的netty 服务端进行通信，获取最新的topic到broker的路由信息
            //进而更新本地缓存的topic路由信息
    ...
    private SendResult sendKernelImpl(final Message msg,
        ...
        if (null == brokerAddr) {
            ...
            //获取要发送的topic 所有在队列  然后队列所在的吧broker 然后在获取对应的broker地址，进行netty通信调用
            //时候进行通道建立
    ...
    public TransactionSendResult sendMessageInTransaction(final Message msg,
        ...
        try {
            //先发消息  然后再回查状态，如果是unknow 则后面broker会回查事务状态
